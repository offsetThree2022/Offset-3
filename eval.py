...
def eval_motion_displacement(model_path=None, save_eval_file_path=None, use_adj_frame_pred=False,
                             ...
                             height_feat_size=13, num_obj_category=5, which_model='OffsetThree'):
    """
    ...
    which_model: [OffsetThree/OffsetThreeMGDA]
    """
    ...
    if which_model == 'OffsetThree':
        model = OffsetThree(out_seq_len=20, motion_category_num=2, height_feat_size=height_feat_size)
        model = nn.DataParallel(model)
        checkpoint = torch.load(model_path)
        model.load_state_dict(checkpoint['model_state_dict'])
        model = model.to(device)
    else:  # OffsetThreeMGDA
...
if __name__ == "__main__":
    ...
    parser.add_argument('-n', '--net', default='OffsetThree', type=str, help='Which network [OffsetThree/OffsetThreeMGDA]')
